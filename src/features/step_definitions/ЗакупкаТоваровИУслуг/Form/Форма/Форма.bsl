...
//&НаКлиенте
//Процедура УдалитьСтарыеТестовыеДокументы()

//	ИмяФайла = ПолучитьПутьКФайлОтносительноКаталогоFeatures("features\Library\step_definitions\УдалениеДокументов.epf");
//	ИмяОбработки = Ванесса.ПодключитьВнешнююОбработкуКлиент(ИмяФайла);
//	ФормаОбработки = ПолучитьФорму("ВнешняяОбработка." + ИмяОбработки + ".Форма.Форма");
//	Если ФормаОбработки = Неопределено Тогда
//		ВызватьИсключение "Форма обработки не найдена";
//	КонецЕсли;
//	
//	ФормаОбработки.ЯУдаляюДокументыВидаПоКонтрагенту("ЗакупкаТоваровИУслуг", "000000002");

//КонецПроцедуры // УдалитьСтарыеТестовыеДокументы()
// 
...
Процедура ЯПодготавливаюВспомогательныеДанные() Экспорт
	...
	//УдалитьСтарыеТестовыеДокументы();
